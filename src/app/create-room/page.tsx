...
export default function CreateRoomPage() {
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-0 relative overflow-hidden">
      ...
      {/* Settings */}
      <div className="relative z-10 flex-1 min-h-0 flex flex-col gap-2.5 px-5 overflow-y-auto pb-2">
        
        {/* Host Name Input */}
        <div className="glass-card rounded-[20px] p-3 flex flex-col gap-1.5">
          <label className="text-violet-300 text-[10px] font-bold uppercase tracking-wider px-1">اسمك (المستضيف)</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="أدخل اسمك هنا..."
            className="bg-white/[0.03] border border-white/[0.08] rounded-xl px-3 py-2 text-white placeholder:text-slate-500 text-sm focus:outline-none focus:border-violet-500/50 transition-colors"
            required
          />
        </div>
      ...
      {/* CTA */}
      <div className="relative z-10 px-5 pt-2 pb-4 sm:pb-6 safe-bottom">
        <button
          onClick={handleCreate}
          disabled={loading}
          className="btn-primary shimmer w-full py-3.5 sm:py-4 rounded-2xl text-white font-bold text-base transition-transform disabled:opacity-50"
        >
